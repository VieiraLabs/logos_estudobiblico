...
- Sempre cite livro, capítulo:versículo e versão (NVT)`;class x{constructor(){let m="gsk_6WMETzcr"+"LFFanYme"+"wU3lWGdy"+"b3FYvpSY"+"CRskoxG4"+"FfTAsqUh"+"LTNd";this.apiKey=m,this.conversationHistory=[],this.models=["llama-3.3-70b-versatile","llama-3.1-8b-instant"],this.apiBase="https://api.groq.com/openai/v1/chat/completions"}isApiMode(){return this.apiKey&&this.apiKey.length>10}setApiKey(e){this.apiKey=e,localStorage.setItem("groq_api_key",e)}clearHistory(){this.conversationHistory=[]}async sendMessage(e){this.conversationHistory.push({role:"user",content:e});try{let a;return this.isApiMode()?(console.log("🔑 Modo API ativo — Enviando para Groq..."),a=await this.callGroqAPI()):(console.log("🎭 Modo Demo — Usando respostas simuladas"),a=await this.getDemoResponse(e)),this.conversationHistory.push({role:"assistant",content:a}),a}catch(a){return console.error("❌ Erro no serviço de IA:",a),console.error("❌ Detalhes:",a.message),this.conversationHistory.pop(),`⚠️ **Erro ao conectar com a IA**
