...
* Use a versão NVT dos textos bíblicos.`;class w{constructor(){let m="gsk_6WMETzcr"+"LFFanYme"+"wU3lWGdy"+"b3FYvpSY"+"CRskoxG4"+"FfTAsqUh"+"LTNd";this.apiKey=m,this.conversationHistory=[],this.models=["llama-3.3-70b-versatile","llama-3.1-8b-instant"],this.apiBase="https://api.groq.com/openai/v1/chat/completions"}isApiMode(){return this.apiKey&&this.apiKey.length>10}setApiKey(e){this.apiKey=e,localStorage.setItem("groq_api_key",e)}clearHistory(){this.conversationHistory=[]}async sendMessage(e){this.conversationHistory.push({role:"user",content:e});try{let a;return this.isApiMode()?(console.log("🔑 Modo API ativo — Enviando para Groq..."),a=await this.callGroqAPI()):(console.log("🎭 Modo Demo — Usando respostas simuladas"),a=await this.getDemoResponse(e)),this.conversationHistory.push({role:"assistant",content:a}),a}catch(a){return console.error("❌ Erro no serviço de IA:",a),console.error("❌ Detalhes:",a.message),this.conversationHistory.pop(),`⚠️ **Erro ao conectar com a IA**
...
Retorne TODOS os versículos do capítulo, sem exceção.`,l=["llama-3.3-70b-versatile","llama-3.1-8b-instant"];for(const b of l)try{const h=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${c}`},body:JSON.stringify({model:b,messages:[{role:"user",content:u}],temperature:.1,max_tokens:8192,response_format:{type:"json_object"}})});if(!h.ok)continue;const g=await h.json(),y=(p=(d=(v=g==null?void 0:g.choices)==null?void 0:v[0])==null?void 0:d.message)==null?void 0:p.content;if(!y)continue;const E=JSON.parse(y);if(E.verses&&E.verses.length>0)return{verses:E.verses,source:"NVT (via IA Groq)",bookName:t,chapter:e}}catch{continue}throw new Error("Groq não conseguiu gerar o texto")}let q=1;function z(){window.addEventListener("open-bible-reader",t=>{const{bookName:e,chapters:a}=t.detail;q=1,P(e,1,a)})}async function P(t,e,a){const o=document.getElementById("chat-messages"),s=document.getElementById("welcome-screen"),r=document.getElementById("chat-title"),i=document.getElementById("chat-status");s&&(s.style.display="none"),r&&(r.textContent=`📖 ${t}`),i&&(i.innerHTML=`<span class="status-dot"></span> Capítulo ${e} de ${a}`),o.innerHTML="";const n=document.createElement("div");n.className="bible-reader",n.id="bible-reader",n.innerHTML=`
  ...
  `,o.appendChild(n),window.lucide&&window.lucide.createIcons(),F(n,t,a),await G(t,e)}function V(t,e){let a="";for(let o=1;o<=t;o++)a+=`<option value="${o}" ${o===e?"selected":""}>Capítulo ${o}</option>`;return a}function F(t,e,a){const o=t.querySelector("#reader-prev"),s=t.querySelector("#reader-next"),r=t.querySelector("#reader-chapter-dropdown");o&&o.addEventListener("click",()=>T(-1,e,a)),s&&s.addEventListener("click",()=>T(1,e,a)),r&&r.addEventListener("change",f=>{q=parseInt(f.target.value),P(e,q,a)});const i=t.querySelector("#reader-prev-bottom"),n=t.querySelector("#reader-next-bottom"),m=t.querySelector("#reader-ask-ai");i&&i.addEventListener("click",()=>T(-1,e,a)),n&&n.addEventListener("click",()=>T(1,e,a)),m&&m.addEventListener("click",()=>{const f=document.getElementById("chat-input"),c=document.getElementById("btn-send"),u=document.getElementById("chat-title"),l=document.getElementById("chat-status");u&&(u.textContent="Assistente Bíblico"),l&&(l.innerHTML='<span class="status-dot"></span> Online — Pronto para ajudar');const v=document.getElementById("chat-messages");v.innerHTML="",f&&(f.value=`Explique ${e} capítulo ${q} com contexto histórico, significado e aplicação prática.`,c.disabled=!1,c.click())})}function T(t,e,a){const o=q+t;o<1||o>a||(q=o,P(e,q,a))}async function G(t,e,a){const o=document.getElementById("reader-content"),s=document.getElementById("reader-nav-bottom");try{const r=await j(t,e);let i=`
